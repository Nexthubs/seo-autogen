...
## R4 后续修复（本地工作树，基线 293a15a）

R4 复验发现的三个 High 已按用户授权修复：全量 Retry 共用 checkpoint 失效逻辑；
revision 血缘限定 seo/fact/style；否定、不确定语气与邻句未验证声明保守降级。
新增离线完整失败恢复测试和语义正负对照；当前完整套件结果为
`560 passed, 48 skipped, 1 warning`。48 项仍为真实数据库/外部服务条件跳过。
详细原问题及关闭证据见本地 `CODEX-AUDIT-REPORT.md` 当前状态节。
不把历史测试或本次词汇筛查扩大为真实部署/完整语义验证。
